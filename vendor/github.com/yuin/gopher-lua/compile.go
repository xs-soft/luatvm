...
func compileLogicalOpExpr(context *funcContext, reg int, expr *ast.LogicalOpExpr, ec *expcontext) { // {{{
	...
	if expr.Operator == "and" {
		compileLogicalOpExprAux(context, reg, expr.Lhs, ec, nextcondlabel, endlabel, false, lb,expr)
		context.SetLabelPc(nextcondlabel, code.LastPC())
		compileLogicalOpExprAux(context, reg, expr.Rhs, ec, endlabel, endlabel, false, lb,expr)
	} else {
		compileLogicalOpExprAux(context, reg, expr.Lhs, ec, endlabel, nextcondlabel, true, lb,expr)
		context.SetLabelPc(nextcondlabel, code.LastPC())
		compileLogicalOpExprAux(context, reg, expr.Rhs, ec, endlabel, endlabel, false, lb,expr)
	}
...
func compileLogicalOpExprAux(context *funcContext, reg int, expr ast.Expr, ec *expcontext, thenlabel, elselabel int, hasnextcond bool, lb *lblabels,fromexpr ast.Expr) { // {{{
	// TODO folding constants?
	code := context.Code
	flip := 0
	jumplabel := elselabel
	//如果是OR.传入左Exp时这个值为true.代表
	if hasnextcond {
		flip = 1
		jumplabel = thenlabel
	}
	...
	case *ast.LogicalOpExpr:
		...
		case "and":
			nextcondlabel := context.NewLabel()
			compileLogicalOpExprAux(context, reg, ex.Lhs, ec, nextcondlabel, elselabel, false, lb,fromexpr)
			context.SetLabelPc(nextcondlabel, context.Code.LastPC())
			compileLogicalOpExprAux(context, reg, ex.Rhs, ec, thenlabel, elselabel, hasnextcond, lb,fromexpr)
		case "or":
			nextcondlabel := context.NewLabel()
			compileLogicalOpExprAux(context, reg, ex.Lhs, ec, thenlabel, nextcondlabel, true, lb,fromexpr)
			context.SetLabelPc(nextcondlabel, context.Code.LastPC())
			compileLogicalOpExprAux(context, reg, ex.Rhs, ec, thenlabel, elselabel, hasnextcond, lb,fromexpr)
		}
		return
	...
	if !hasnextcond && thenlabel == elselabel {
		...
		reg += compileExpr(context, reg, expr, ecnone(0))
		var fromOr=false
		switch fex := fromexpr.(type) {
		case *ast.LogicalOpExpr:
			if fex.Operator=="or" && !hasnextcond{
				fromOr=true
			}
		default:
			fmt.Println("上层对象",fex)
		}
		if sreg == a ||fromOr{
			//判定
			code.AddABC(OP_TEST, a, 0, 0^flip, sline(expr))
