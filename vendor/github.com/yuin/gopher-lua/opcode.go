...
func opToString(inst uint32) string {
	...
	case OP_CALL:
		buf += fmt.Sprintf(";调用函数; R(%v) ... R(%v+%v-2) := R(%v)(R(%v+1) ... R(%v+%v-1))", arga, arga, argc, arga, arga, arga, argb)
	case OP_TAILCALL:
	...
	case OP_CLOSURE:
		buf += fmt.Sprintf(";闭包; R(%v) := closure(KPROTO[%v] R(%v) ... R(%v+n))", arga, argbx, arga, arga)
	case OP_VARARG:
