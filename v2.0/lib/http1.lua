...
local function taskClient(method,protocal,auth,host,port,path,cert,head,body,timeout,cbFnc,rcvFilePath)
    ...
    local rcvCache,rspHead,rspBody,d1,d2,result,data,StateCode,rcvChunked,contentLen = "",{},{}
    --接收数据，解析状态行和头
    while true do
        ...
        if d2 then
            --状态行
            _,d1,StateCode = rcvCache:find("%s(%d+)%s.-\r\n")
            if not StateCode then
                return response(client,cbFnc,false,"parse received State error",rspHead,rcvFilePath or table.concat(rspBody))
            end
            --应答头
    ...
    config.set('OTA',1)
    print('rcvChunked->',rcvChunked)
    --解析body
    if rcvChunked then
        local chunkSize
        local chunkLoad=0
        local chunkLoadSum=0
        local chunkSizeSum=0
        --循环处理每个chunk
        while true do
            --解析chunk size
            if not chunkSize then
                ...
                if chunkSize then
                    ...
                    chunkSizeSum=chunkSizeSum+chunkSize
                    --缓存
                    rcvCache = rcvCache:sub(d2+1,-1)
                else
                    result,data = receive(client,timeout,cbFnc,false,nil,rspHead,rcvFilePath or table.concat(rspBody))
            ...
            --解析chunk data
            if chunkSize then
                log.info("进行处理,块长度",chunkSize)
                if chunkSize==0 then
                    log.info("累计写",chunkLoadSum)
                    local userResult = rcvFilePath("",-2,StateCode)
                    if userResult~=nil then
                        return response(client,cbFnc,userResult,userResult and StateCode or "receive: user process error",rspHead)
                    end
                    return response(client,cbFnc,true,StateCode,rspHead,rcvFilePath or table.concat(rspBody))
                end
                --回调处理模式
                if type(rcvFilePath)=="function" then

                    if chunkLoad+rcvCache:len()<chunkSize+2 then
                        result,data = receive(client,timeout,cbFnc,false,nil,rspHead,rcvFilePath or table.concat(rspBody))
                        if not result then return end
                        --chunkLoad=chunkLoad+data:len()
                        --保存data到文件
                        rcvCache = rcvCache ..data
                        --如果数据小于快大小则直接读取数据
                        if chunkLoad+rcvCache:len()<=chunkSize then
                            chunkLoad=chunkLoad+rcvCache:len()
                            chunkLoadSum=chunkLoadSum+rcvCache:len()
                            local userResult = rcvFilePath(rcvCache,-1,StateCode)
                            if userResult~=nil then
                                return response(client,cbFnc,userResult,userResult and StateCode or "receive: user process error",rspHead)
                            end
                            rcvCache=""
                        ...
                        if chunkSize>0 then
                            local chunkData = rcvCache:sub(1,chunkSize-chunkLoad)
                            local userResult = rcvFilePath(chunkData,-1,StateCode)
                            if userResult~=nil then
                                return response(client,cbFnc,userResult,userResult and StateCode or "receive: user process error",rspHead)
                            end
                            --保存到文件中
                            chunkLoadSum=chunkLoadSum+chunkData:len()
                            rcvCache = rcvCache:sub(chunkSize-chunkLoad+3,-1)
                            chunkLoad=0
                            --chunkLoad =0
                            chunkSize = nil
                        end
        ...
        while true do
            ...
            elseif type(rcvFilePath)=="function0" then
                local userResult = rcvFilePath(data,rspHead["Content-Range"] and tonumber((rspHead["Content-Range"]):match("/(%d+)")) or contentLen,StateCode)
                if userResult~=nil then
                    return response(client,cbFnc,userResult,userResult and StateCode or "receive: user process error",rspHead)
                end
            else
            ...
            if rmnLen==0 then break end
                result,rcvCache = receive(client,timeout,cbFnc,contentLen==0x7FFFFFFF,contentLen==0x7FFFFFFF and StateCode or nil,rspHead,rcvFilePath or table.concat(rspBody))
            if not result then return end
        end
        return response(client,cbFnc,true,StateCode,rspHead,rcvFilePath or table.concat(rspBody))
    end
